<script lang="ts">
	...
	let filter = ''
	$: popular = projects.popular.filter(
		(s) =>
			s.name.toLowerCase().includes(filter.toLowerCase()) ||
			s.description?.toLowerCase().includes(filter.toLowerCase()) ||
			s.topics.filter((c) => c.toLowerCase().includes(filter.toLowerCase())).length
	)
	$: recent = projects.recent.filter(
		(s) =>
			s.name.toLowerCase().includes(filter.toLowerCase()) ||
			s.description?.toLowerCase().includes(filter.toLowerCase()) ||
			s.topics.filter((c) => c.toLowerCase().includes(filter.toLowerCase())).length
...
<Section title="projects">
	<div class="p-10 lg:flex lg:p-0 rounded-xl overflow-hidden card mt-5">
		<img
	...
	<div class="p-10 lg:flex lg:p-0 rounded-xl overflow-hidden card mt-5">
		<img class="w-16 h-16 lg:mt-5 lg:ml-5" src={zoiImg} alt="Zoi" />
		<div class="pt-6 lg:p-5 items-center">
			<p class="text-justify font-bold">Zoi</p>
			<p class="text-justify font-bold">Full-Stack Developer assigned to Digital Innovations.</p>
			<p class="text-left text-xs text-gray-300">06/2021 - Current</p>
			<p class="text-justify pt-5">
				Worked close to partners as a developer in exploratory projects using innovative cloud
	...
	<div class="p-10 lg:flex lg:p-0 rounded-xl overflow-hidden card mt-5">
		<img class="w-16 h-16 lg:mt-5 lg:ml-5" src={iqaImg} alt="IQA" />
		<div class="pt-6 lg:p-5 items-center">
			<p class="text-justify font-bold">IQA</p>
			<p class="text-justify font-bold">
				Full-Stack Developer of multiple projects using many different technologies.
			</p>
			<p class="text-left text-xs text-gray-300">03/2011 - 05/2021</p>
			<p class="text-justify pt-5">
				Developed AGIR ®, a Quality Management Software (QMS), Process Management (BPM) and Document
